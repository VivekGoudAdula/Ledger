...
import { Activity, Radio, PauseCircle, PlayCircle } from 'lucide-react';

export function Header({ systemStatus, connectionStatus }) {
  ...
  useEffect(() => {
    async function checkStatus() {
      try {
        ...
        console.warn('Error fetching ingestion status:', err);
      }
    }
    checkStatus();
  }, []);
  ...
  return (
    <header className="header">
      <div className="brand" style={{ display: 'flex', alignItems: 'center' }}>
        <img src="/logo.png" alt="Ledger Logo" className="brand-logo" style={{ height: '36px', marginRight: '12px', borderRadius: '4px' }} />
        <div>
      ...
      <div className="status-badge-container" style={{ gap: '10px' }}>
        <button
        ...
        <span className={`ws-badge ${connectionStatus === 'LIVE STREAM' || connectionStatus === 'CONNECTED' ? 'live' : connectionStatus === 'RECONNECTING' || connectionStatus === 'CONNECTING' ? 'connecting' : 'disconnected'}`}>
